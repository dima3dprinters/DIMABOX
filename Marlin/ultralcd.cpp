...
int flexPreheatHotendTemp;
int flexPreheatHPBTemp;
int flexPreheatFanSpeed;
...
static void lcd_prepare_preheat_menu();
static void lcd_homing_menu();
static void lcd_filament_retire();
static void lcd_filament_insert();
//static void lcd_changefilament_menu();
...
static void menu_action_setting_edit_float12(const char* pstr, float* ptr, float minValue, float maxValue);
...
#if TEMP_SENSOR_0 != 0
    //MENU_ITEM_EDIT(int3, MSG_TEMPERATURE, &target_temperature[0], 0, HEATER_0_MAXTEMP - 15);
#endif
#if TEMP_SENSOR_1 != 0
...
void lcd_preheat_madera()
{
    setTargetHotend0(maderaPreheatHotendTemp);
    lcd_return_to_status();
    setWatch(); // heater sanity check timer
}
...
void lcd_preheat_flex()
{
    ...
    setTargetHotend1(flexPreheatHotendTemp);
    setTargetHotend2(flexPreheatHotendTemp);
...
static void lcd_prepare_menu()
{
    ...
    #endif
#endif
    MENU_ITEM(submenu,MSG_PREHEAT, lcd_prepare_preheat_menu);
	MENU_ITEM(function, MSG_COOLDOWN, lcd_cooldown);
    MENU_ITEM(submenu, MSG_RETIRARFILA, lcd_filament_retire);
    MENU_ITEM(submenu, MSG_INSERTARFILA, lcd_filament_insert);
	//MENU_ITEM(submenu,MSG_CHANGE_FILAMENT, lcd_changefilament_menu);
...
static void lcd_prepare_preheat_menu()
{
    ...
    MENU_ITEM(function, MSG_PREHEAT_MADERA, lcd_preheat_madera);
...
{
    START_MENU();
    MENU_ITEM(back, MSG_PREPARE, lcd_control_menu);
...
//			u8g.drawStr(7,15,"Cambio de filamento");
//			//u8g.setFont(u8g_font_5x8);
//			u8g.drawStr(10,30,"¿Desea extraer el ");
//            u8g.drawStr(10,39,"filamento actual?");
//            //MENU_ITEM(submenu,MSG_PREHEAT, lcd_prepare_preheat_menu);
//	   } while( u8g.nextPage() );
//	   while(5);
//    END_MENU();
...
static void lcd_filament_retire()
{
    ...
    enquecommand_P(PSTR("G1 E-100 F1200"));
    enquecommand_P(PSTR("G92 E0"));
...
{
    enquecommand_P(PSTR("G92 E0"));
    enquecommand_P(PSTR("G1 E+50 F1200"));
    enquecommand_P(PSTR("G92 E0"));
    lcd_goto_menu(lcd_prepare_menu);
}
...
static void lcd_control_menu()
{
    ...
    MENU_ITEM(submenu,MSG_HOME, lcd_homing_menu);

    MENU_ITEM(submenu, "Ajustar Z-offset", lcd_control_zoffset);
...
static void lcd_control_zoffset()
{
    START_MENU();
    MENU_ITEM(back, MSG_CONTROL, lcd_control_menu);
    MENU_ITEM_EDIT(float12, "Z-offset", &zprobe_zoffset, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX);
    MENU_ITEM(function, "Guardar valor", Config_StoreSettings);
    END_MENU();
}
...
#endif
    MENU_ITEM(function, MSG_RESTORE_FAILSAFE, Config_ResetDefault);
    MENU_ITEM(function, "Controles Temp.", lcd_control_temperature_menu);
    END_MENU();
}
...
{
    START_MENU();
    MENU_ITEM(back, MSG_ADVANCED, lcd_control_advanced_menu);
...
menu_edit_type(float, float12, ftostr12, 100)
